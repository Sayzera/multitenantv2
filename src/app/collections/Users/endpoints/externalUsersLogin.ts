...
export const externalUsersLogin: Endpoint = {
  handler: async (req) => {
    ...
    const { password, tenantSlug, tenantDomain, email } = data

    if (!email || !password) {
      throw new APIError('email and Password are required for login.', 400, null, true)
    }

    const currentUser = await req.payload.find({
      collection: 'users',
      where: {
        email: {
          equals: email,
    ...
    const tenant = Array.isArray(currentUser.docs[0].tenants)
      ? currentUser.docs[0].tenants[0].tenant
      : currentUser.docs[0].tenants?.tenant
